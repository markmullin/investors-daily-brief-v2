...
// Enhanced batch import with proper position handling
router.post('/:id/transactions/batch', (req, res) => {
  ...
  console.log(`\n=== Batch import: ${transactions.length} transactions for account: ${accountName}, merge mode: ${mergeMode} ===`);
  ...
  // Handle replace mode for position imports specially
  if (mergeMode === 'replace' && isPositionImport) {
    console.log(`\nHandling position import with replace mode for account: ${accountName}`);
    ...
    symbolsToSell.forEach(symbol => {
      ...
      if (accountData && accountData.quantity > 0) {
        ...
        holding.totalCost = holding.quantity * holding.avgCost;
  ...
  } else if (clearExisting || mergeMode === 'replace') {
    // Original replace logic for transaction imports
    if (mergeMode === 'replace') {
      // Only clear data for this specific account
      portfolio.transactions = portfolio.transactions.filter(t => t.account !== accountName);
      
      // Rebuild holdings from remaining transactions
      portfolio.holdings = {};
      const remainingTransactions = portfolio.transactions;
      
      remainingTransactions.forEach(txn => {
        if (!portfolio.holdings[txn.symbol]) {
        ...
        if (!holding.accounts[txn.account]) {
          holding.accounts[txn.account] = { quantity: 0, totalCost: 0 };
        }
        
        if (txn.action === 'BUY') {
          const newTotalCost = holding.totalCost + txn.total;
          const newQuantity = holding.quantity + txn.quantity;
          holding.quantity = newQuantity;
          holding.totalCost = newTotalCost;
          holding.avgCost = holding.totalCost / holding.quantity;
          holding.accounts[txn.account].quantity += txn.quantity;
          holding.accounts[txn.account].totalCost += txn.total;
        } else if (txn.action === 'SELL') {
          holding.quantity -= txn.quantity;
          holding.accounts[txn.account].quantity -= txn.quantity;
          
          if (holding.quantity <= 0.001) {
            delete portfolio.holdings[txn.symbol];
          } else {
            holding.totalCost = holding.avgCost * holding.quantity;
          }
        }
  ...
  // Log summary
  console.log(`\nImport complete: ${successCount} transactions imported for account ${accountName}`);
  ...
  res.json({ 
    message: `Imported ${successCount} transactions successfully for ${accountName}`,
    successCount,
    errors,
